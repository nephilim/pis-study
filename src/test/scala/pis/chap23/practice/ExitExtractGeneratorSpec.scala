...
class ExifExtractorGeneratorSpec extends Spec {
  val DIR_NAME = "/Users/skplanet/images"
  ...
  describe("Metadata 추출") {
    ...
    it("GPS 정보 추출") {
      ...
      assert(extractor.extractGpsPos(metadata).toString 
          == "(37.0° 32.0\' 5.399999999993383\",126.0° 59.0\' 39.59999999998786\")")
    }
    it("Generator 동작확인") {
      val extractor = new ExifGifExtractor(DIR_NAME)
      for( position <- extractor ) {	// position: Tuple[String,String]
        Console.println("위도 %s, 경도 %s".format(position._1, position._2))
      }
    }
  }
